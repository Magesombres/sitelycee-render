...
const Char    socket.on('move', asy    socket.on('move', async (data) => {
      try {
        ...
        const { dx, dy } = validated;
        
        const z = getZone(currentZone);
        const rec = z.get(socket.id);
        if (!rec) return;
        
        const nx = clamp(rec.x + dx);
        const ny = clamp(rec.y + dy);
        rec.x = nx; rec.y = ny;
        z.set(socket.id, rec);
        ...
        try { 
          await Character.updateOne({ user: user.id }, { $set: { x: nx, y: ny, lastSeen: new Date() } }); 
        } catch {}
      } catch (e) {
...
module.exports = function attachMMO(io) {
  ...
  nsp.on('connection', async (socket) => {
    ...
    socket.on('change_zone', async (data) => {
      try {
        const validated = changeZoneSchema.parse(data);
        const target = validated.zone;
        ...
        try { 
          await Character.updateOne({ user: user.id }, { $set: { zone: currentZone, x: 10, y: 10, lastSeen: new Date() } }); 
        } catch {}
      } catch (e) {
        if (e instanceof z.ZodError) {
          socket.emit('error_msg', 'Donn├®es invalides: ' + e.errors[0].message);
