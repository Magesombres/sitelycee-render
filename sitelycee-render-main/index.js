...
const apiLimiter = rateLimit({
  ...
  max: Number(process.env.RATE_LIMIT_MAX || 150), // Réduit de 300 à 150 pour éviter les abus
...
console.log('🔧 Configuration:', {
  ...
  port: PORT,
  mongoUri: MONGO_URI.replace(/\/\/([^:]+):([^@]+)@/, '//$1:****@'), // Masquer le mot de passe dans les logs
});
...
if (process.env.NODE_ENV === 'production' && !process.env.MONGO_URI) {
  ...
  console.error('ℹ️  Example: mongodb+srv://username:password@cluster.mongodb.net/sitelyee?retryWrites=true&w=majority');
  process.exit(1);
}
...
mongoose
  ...
  .then(() => {
    ...
    // Bind sur 0.0.0.0 pour que Render puisse accéder au serveur
  ...
  .catch((err) => {
    ...
    console.error('ℹ️  Vérifiez que MONGO_URI est correct et que MongoDB est accessible');
    process.exit(1);
  });
